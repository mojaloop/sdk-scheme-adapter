...
import { DomainEvent, IProcessSDKOutboundBulkAcceptQuoteCmdEvtData, ProcessSDKOutboundBulkAcceptQuoteCmdEvt, SDKOutboundBulkAcceptQuoteReceivedDmEvt } from '@mojaloop/sdk-scheme-adapter-private-shared-lib';
import { IDomainEventHandlerOptions } from '../../types';

export async function handleSDKOutboundBulkAcceptQuoteReceived(
    message: DomainEvent,
    options: IDomainEventHandlerOptions,
    logger: ILogger,
): Promise<void> {
    const sdkOutboundBulkAcceptPartyInfoReceived
        = SDKOutboundBulkAcceptQuoteReceivedDmEvt.CreateFromDomainEvent(message);
    try {
        const processSDKOutboundBulkAcceptQuoteCmdEvtData: IProcessSDKOutboundBulkAcceptQuoteCmdEvtData = {
            ...
            bulkTransactionContinuationAcceptQuote: sdkOutboundBulkAcceptPartyInfoReceived.getBulkTransactionContinuationAcceptQuote(),
            timestamp: Date.now(),
            headers: [],
        };
        const processSDKOutboundBulkAcceptPartyInfoMessage
            = new ProcessSDKOutboundBulkAcceptQuoteCmdEvt(processSDKOutboundBulkAcceptQuoteCmdEvtData);
