 ...
 ******/
 ...
 describe("Tests for ProcessSDKOutboundBulkTransferRequestCmdEvnt Command Event", () => {
   ...
   test("When inbound command event ProcessSDKOutboundBulkTransferRequestCmdEvnt is received \
         Then the global state should be updated to TRANSFERS_PROCESSING \
         And the logic should loop through quote batches that were created as part of bulk quote processing \
         And create a new transfer batch to only include transfers whose quote status was AGREMENT_ACCEPTED \
         And update the individual transfer batch state to TRANSFERS_PROCESSING \
         And BulkTransfersRequestedDmEvnt should be published for each transfer batch", async () => {
