...
class OutboundBulkTransfersModel {
    ...
    async _handleTransition(lifecycle, ...args) {
        this._logger.debug(`Bulk transfer ${this.data.bulkTransferId} is transitioning from ${lifecycle.from} to ${lifecycle.to} in response to ${lifecycle.transition}`);
    ...
    async _executeBulkTransfer() {
        ...
        return new Promise(async (resolve, reject) => {
            ...
            const subId = await this._cache.subscribe(bulkTransferKey, (cn, msg, subId) => {
                try {
                    ...
                    if (message.type === 'bulkTransferResponse') {
                        ...
                        this._logger.push({ message }).debug(`Ignoring cache notification for bulk transfer ${bulkTransferKey}. Unknown message type ${message.type}.`);
                        return;
                    }
                    ...
                    this._cache.unsubscribe(bulkTransferKey, subId).catch(e => {
                        this._logger.error(`Error unsubscribing (in callback) ${bulkTransferKey} ${subId}: ${e.stack || safeStringify(e)}`);
                    });
                    ...
                    this._logger.push({ bulkTransferFulfil }).debug('Bulk transfer fulfils received');
            ...
            const timeout = setTimeout(() => {
                ...
                // we dont really care if the unsubscribe fails but we should log it regardless
                this._cache.unsubscribe(bulkTransferKey, subId).catch(e => {
                    this._logger.error(`Error unsubscribing (in timeout handler) ${bulkTransferKey} ${subId}: ${e.stack || safeStringify(e)}`);
                });
            ...
            try {
                ...
                this._logger.push({ res }).debug('Bulk transfer request sent to peer');
            }
            catch (err) {
                ...
                // we dont really care if the unsubscribe fails but we should log it regardless
                this._cache.unsubscribe(bulkTransferKey, subId).catch(e => {
                    this._logger.error(`Error unsubscribing (in error handler) ${bulkTransferKey} ${subId}: ${e.stack || safeStringify(e)}`);
                });
    ...
    async _getBulkTransfer(bulkTransferId) {
        ...
        return new Promise(async (resolve, reject) => {
            ...
            const subId = await this._cache.subscribe(bulkTransferKey, (cn, msg, subId) => {
                try {
                    ...
                    if (message.type === 'bulkTransferResponseError') {
                        ...
                        this._logger.push({ message }).debug(`Ignoring cache notification for bulk transfer ${bulkTransferKey}. Uknokwn message type ${message.type}.`);
                        return;
                    }
                    ...
                    this._logger.push({ fulfils }).debug('Bulk transfer fulfils received');
            ...
            try {
                ...
                this._logger.push({ peer: res }).debug('Bulk transfer lookup sent to peer');
            }
            catch(err) {
                ...
                // we dont really care if the unsubscribe fails but we should log it regardless
                this._cache.unsubscribe(bulkTransferKey, subId).catch(e => {
                    this._logger.error(`Error unsubscribing ${bulkTransferKey} ${subId}: ${e.stack || safeStringify(e)}`);
                });
    ...
    getResponse() {
        ...
        switch(this.data.currentState) {
            ...
            default:
                this._logger.debug(`Bulk transfer model response being returned from an unexpected state: ${this.data.currentState}. Returning ERROR_OCCURRED state`);
                resp.currentState = SDKStateEnum.ERROR_OCCURRED;
                break;
        }
    ...
    async _save() {
        try {
            ...
            this._logger.push({ res }).debug('Persisted bulk transfer model in cache');
        }
        catch(err) {
            this._logger.push({ err }).error('Error saving bulk transfer model');
            throw err;
        }
    }
    ...
    async load(bulkTransferId) {
        try {
            ...
            this._logger.push({ cache: this.data }).debug('Bulk transfer model loaded from cached state');
        }
        catch(err) {
            this._logger.push({ err }).error('Error loading bulk transfer model');
            throw err;
        }
    }
        ...
        try {
            ...
            switch(this.data.currentState) {
                case 'start':
                    ...
                    this._logger.debug(`Bulk transfer ${this.data.bulkTransferId} has been completed`);
                    break;

                case 'getBulkTransfer':
                    await this.stateMachine.getBulkTransfer();
                    this._logger.debug(`Get bulk transfer ${this.data.bulkTransferId} has been completed`);
                    break;

                case 'succeeded':
                    // all steps complete so return
                    this._logger.debug('Bulk transfer completed successfully');
                    await this._save();
            ...
            this._logger.debug(`Bulk transfer model state machine transition completed in state: ${this.stateMachine.state}. Recursing to handle next transition.`);
            return this.run();
        }
        catch(err) {
