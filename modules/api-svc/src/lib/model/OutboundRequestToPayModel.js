...
class OutboundRequestToPayModel {
    ...
    async _handleTransition(lifecycle, ...args) {
        this._logger.debug(`Request To Pay ${this.data.transactionRequestId} is transitioning from ${lifecycle.from} to ${lifecycle.to} in response to ${lifecycle.transition}`);
    ...
    async _executeTransactionRequest() {
        ...
        return new Promise(async (resolve, reject) => {
            ...
            const subId = await this._cache.subscribe(transactionRequestKey, async (cn, msg, subId) => {
                try {
                    ...
                    this._cache.unsubscribe(transactionRequestKey, subId).catch(e => {
                        this._logger.error(`Error unsubscribing (in callback) ${transactionRequestKey} ${subId}: ${e.stack || safeStringify(e)}`);
                    });
            ...
            const timeout = setTimeout(() => {
                ...
                // we dont really care if the unsubscribe fails but we should log it regardless
                this._cache.unsubscribe(transactionRequestKey, subId).catch(e => {
                    this._logger.error(`Error unsubscribing (in timeout handler) ${transactionRequestKey} ${subId}: ${e.stack || safeStringify(e)}`);
                });
            ...
            try {
                ...
                // we dont really care if the unsubscribe fails but we should log it regardless
                this._cache.unsubscribe(transactionRequestKey, subId).catch(e => {
                    this._logger.error(`Error unsubscribing (in error handler) ${transactionRequestKey} ${subId}: ${e.stack || safeStringify(e)}`);
                });
    ...
    async load(transactionRequestId) {
        try {
            ...
            this._logger.push({ cache: this.data }).debug('TransactionRequest model loaded from cached state');
        }
        catch(err) {
        ...
        try {
            ...
            switch(this.data.currentState) {
                ...
                case 'payeeResolved':
                    ...
                    this._logger.debug(`Transaction Request for ${this.data.transactionRequestId} has been completed`);
                    break;

                case 'succeeded':
                    // all steps complete so return
                    this._logger.debug('Transaction Request completed successfully');
                    await this._save();
                    return this.getResponse();
            ...
            this._logger.debug(`Transfer model state machine transition completed in state: ${this.stateMachine.state}. Recursing to handle next transition.`);
            return this.run();
        }
        catch(err) {
    ...
    getResponse() {
        ...
        switch(this.data.currentState) {
            ...
            default:
                this._logger.debug(`Transaction Request model response being returned from an unexpected state: ${this.data.currentState}. Returning ERROR_OCCURRED state`);
                resp.currentState = SDKStateEnum.ERROR_OCCURRED;
                break;
        }
    ...
    async _save() {
        try {
            ...
            this._logger.push({ res }).debug('Persisted transaction request model in cache');
        }
        catch(err) {
