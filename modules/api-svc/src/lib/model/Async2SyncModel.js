...
function generate({
        ...
        try {
            ...
            switch (data.currentState) {
                ...
                case 'succeeded':
                    ...
                    logger.debug('Action called successfully');
                    return this.getResponse();

                case 'errored':
                    // stopped in errored state
                    logger.error('State machine in errored state');
                    return;
            }
        } catch (err) {
            logger.error(`Error running ${modelName} model: ${safeStringify(err)}`);

            // as this function is recursive, we don't want to error the state machine multiple times
            if (data.currentState !== 'errored') {
                // err should not have a requestActionState property here!
                if (err.requestActionState) {
                    logger.debug('State machine is broken');
                }
    ...
    async function onRequestAction(fsm, args) {
        ...
        logger.push({ args }).debug('onRequestAction - arguments');

        return deferredJob(cache, channelNameMethod(args))
            .init(async (channel) => {
                const res = await requestActionMethod(requests, args);
                logger.push({ res, channel, args }).debug('RequestAction call sent to peer, listening on response');
                return res;
            })
            .job((message) => {
                this.context.data = {
                ...
                logger.push({ message }).debug('requestActionMethod message received');
            })
