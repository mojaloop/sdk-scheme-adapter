...
class OutboundTransfersModel {
    ...
    async _handleTransition(lifecycle, ...args) {
        ...
        switch(lifecycle.transition) {
            ...
            case 'abort':
                this._logger.debug('State machine is aborting transfer');
    ...
    _resolveBatchPayees() {
        ...
        return new Promise(async (resolve, reject) => {
            ...
            const timer = setTimeout(async () => {
                ...
                this._logger.push({ payeeList }).debug('Payees resolved');
            ...
            try {
                ...
                this.data.getPartiesRequest = res.originalRequest;
                this.metrics.partyLookupRequests.inc();
                this._logger.push({ peer: res }).error('Party lookup sent to peer');
            }
            catch(err) {
        ...
        return new Promise(async (resolve, reject) => {
            ...
            const subId = await this._cache.subscribe(transferKey, async (cn, msg, subId) => {
                try {
                    ...
                    this._logger.push({ fulfil: fulfil.body }).debug('Transfer fulfil received');
                    ...
                    if(this._sendFinalNotificationIfRequested && fulfil.body.transferState === FSPIOPTransferStateEnum.RESERVED) {
                        ...
                        this._logger.debug(`PATCH final notification sent to peer for transfer ${this.data.transferId}`);
                    }
            ...
            const subId = await this._cache.subscribe(transferKey, (cn, msg, subId) => {
                try {
                    ...
                    if (message.type === 'transferError') {
                        ...
                        this._logger.push({ message }).debug(`Ignoring cache notification for transfer ${transferKey}. Uknokwn message type ${message.type}.`);
                        return;
                    }
                    // cancel the timeout handler
                    clearTimeout(timeout);
    ...
    async load(transferId) {
        try {
            ...
            this._logger.push({ cache: this.data }).debug('Transfer model loaded from cached state');
        }
        catch(err) {
            this._logger.push({ err }).debug('Error loading transfer model');
            throw err;
        }
    }
    ...
    async run(mergeData) {
        try {
            ...
            switch(this.data.currentState) {
                case 'start':
                    ...
                    if(typeof(this.data.to.fspId) !== 'undefined' && this.data.skipPartyLookup) {
                        ...
                        this._logger.debug(`Skipping payee resolution for transfer ${this.data.transferId} as to.fspId was provided and skipPartyLookup is truthy`);
                    ...
                    this._logger.debug(`Payee resolved for transfer ${this.data.transferId}`);
                ...
                case 'getTransfer':
                    ...
                    this._logger.debug(`Get transfer ${this.data.transferId} has been completed`);
                    break;
                ...
                case 'aborted':
                    ...
                    await this._save();
                    this._logger.debug('State machine in aborted state');
                    return this.getResponse();
            ...
            this._logger.debug(`Transfer model state machine transition completed in state: ${this.stateMachine.state}. Recusring to handle next transition.`);
            return this.run();
        }
        catch(err) {
