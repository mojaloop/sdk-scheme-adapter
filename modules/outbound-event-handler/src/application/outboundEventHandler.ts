...
import { TransfersTopics } from '@mojaloop/sdk-scheme-adapter-public-messages-lib'
import { IRunHandler, KafkaDomainEventsConsumer } from '@mojaloop/sdk-scheme-adapter-infra-lib'
import { IEventsConsumer, DomainEventMessage } from '@mojaloop/sdk-scheme-adapter-private-types-lib';
...
export class OutboundEventHandler implements IRunHandler {
  ...
  async _messageHandler (message: DomainEventMessage): Promise<void> {
    this._logger.isInfoEnabled() && this._logger.info(`outboundEvtHandler - ${message}`)
    console.log(message)
    // try {
