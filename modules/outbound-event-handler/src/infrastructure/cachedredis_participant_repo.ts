...
export class CachedRedisParticipantStateRepo implements IParticipantRepo {
  ...
  async getAllIds (): Promise<string[]> {
    ...
    return await new Promise(async (resolve, reject) => {
      this._logger.isDebugEnabled() && this._logger.debug('CachedRedisParticipantStateRepo::getAll() - start')
      ...
      this._redisClient.keys(this.keyPrefix + '*', (err: Error | null, result: string[]) => {
        ...
        this._logger.isDebugEnabled() && this._logger.debug(`CachedRedisParticipantStateRepo::getAll() - got back, ${result.length} results'`)
        let results: string[]
