...
class OutboundBulkTransfersModel {
    ...
    async _requestBulkQuote() {
        ...
        return new Promise(async (resolve, reject) => {
            ...
            const timeout = setTimeout(() => {
                const err = new BackendError(`Timeout requesting quote for transfer ${this.data.bulkTransferId}`, 504);
    ...
    _buildQuoteRequest() {
        let quote = {
            quoteId: uuid(),
            transactionId: this.data.bulkTransferId,
    ...
    async _save() {
        try {
            this.data.currentState = this.stateMachine.state;
            const res = await this._cache.set(`transferModel_${this.data.bulkTransferId}`, this.data);
    ...
    async load(bulkTransferId) {
        try {
            const data = await this._cache.get(`transferModel_${bulkTransferId}`);
            if(!data) {
